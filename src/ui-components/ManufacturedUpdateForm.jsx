...
export default function ManufacturedUpdateForm(props) {
  const {
    id,
    manufactured,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...manufacturedRecord };
  ...
  const [manufacturedRecord, setManufacturedRecord] =
    React.useState(manufactured);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(Manufactured, id)
        : manufactured;
      setManufacturedRecord(record);
    };
    queryData();
  }, [id, manufactured]);
  React.useEffect(resetStateValues, [manufacturedRecord]);
  const validations = {
    Barcode: [],
    Electricity: [],
    Renewable: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            Manufactured.copyOf(manufacturedRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "ManufacturedUpdateForm")}
    >
      <TextField
        label="Barcode"
        isRequired={false}
